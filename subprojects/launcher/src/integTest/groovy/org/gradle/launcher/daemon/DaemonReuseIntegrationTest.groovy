...
class DaemonReuseIntegrationTest extends DaemonIntegrationSpec {
    ...
    def "handles two clients that attempt to connect to an idle daemon simultaneously"() {
        ...
        and:
        assert !gradle1.standardOutput.contains("Could not receive message from client")
        assert !gradle2.standardOutput.contains("Could not receive message from client")
    }
